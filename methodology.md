...
1. **Phase 1** — Baseline evaluation of an undefended (compressed-only) model across two compression levels (INT8, INT4) and three attack types.
...
4. **Phase 3** — The combined sequential attack (FGSM → PGD → Patch) applied against the undefended, AT, and AT+KD models at each compression level.
